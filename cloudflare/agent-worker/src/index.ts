...
const TOOLS = [
  ...
  { type: "function", function: { name: "ucp_create_checkout",
    description: "POST /checkout-sessions — 创建结账会话，返回 checkout_id 和价格。buyer_email 可选，缺省用默认值，无需向用户询问。",
    parameters: { type: "object",
      properties: {
        token: { type: "string" }, lane_id: { type: "string" },
        buyer_email: { type: "string", description: "买家邮箱，可选，缺省 guest@vending.local" },
        machine_id: { type: "string", description: "贩卖机 ID，如 vm-001。默认 vm-001。" },
      },
